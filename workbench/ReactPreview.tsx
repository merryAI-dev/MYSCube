...
export type ReactPreviewArtifact = { bundle: string; css: string; sourceHash: string; bundleHash: string; cssHash: string; runtimeVersion: string; packageSetHash: string; executionId?: string };
export type ReactPreviewResult = { status: 'ready' | 'error'; message?: string; durationMs?: number; executionId?: string };
export type ReactPreviewProps = { artifact: ReactPreviewArtifact | null; runtimeUrl?: string | null; onApiCall: (apiId: string, input: unknown) => Promise<unknown>; onResult?: (result: ReactPreviewResult) => void };
...
export function ReactPreview({ artifact, runtimeUrl, onApiCall, onResult }: ReactPreviewProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      if (artifact.runtimeVersion !== 'react-preview-v1' || !/^[a-f0-9]{64}$/.test(artifact.packageSetHash) || !/^[a-f0-9]{64}$/.test(artifact.sourceHash) || typeof artifact.bundle !== 'string' || typeof artifact.css !== 'string' || new TextEncoder().encode(artifact.bundle).byteLength > 240000 || new TextEncoder().encode(artifact.css).byteLength > 80000 || await hash(artifact.bundle) !== artifact.bundleHash || await hash(artifact.css) !== artifact.cssHash) throw new Error('React 실행본의 내용 또는 버전이 일치하지 않습니다.');
  ...
  return <section aria-label="React 미리보기" style={{ minWidth: 0 }}><p role="status" style={{ color: '#64748b', fontSize: 12 }}>{preparing ? '새 React 화면을 확인하고 있습니다…' : committedId ? '독립 실행 공간의 React 미리보기' : 'React 실행본을 준비해 주세요.'}</p>
    <p style={{ color: '#64748b', fontSize: 12 }}>API는 등록된 항목과 현재 권한으로만 요청합니다. 무제한 자원 격리를 보장하는 실행 환경은 아닙니다.</p>

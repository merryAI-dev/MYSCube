...
test.beforeAll(async () => {
  ...
  server = createWorkbenchApp({ db, env, now, authMode: 'headers', reactCompletionFactory: () => async () => ({ tool_calls: [{ function: { name: 'render_react_source', arguments: JSON.stringify(source) } }] }) }).listen(0, '127.0.0.1');
...
const generate = async (page: any) => { await page.getByLabel('React 화면 요청').fill('자료 없이 카운터와 제목 컴포넌트를 여러 파일로 나눈 화면을 만들어 주세요'); await page.getByRole('button', { name: 'React 생성·수정 요청' }).click(); await expect(page.getByRole('button', { name: 'React 제안 적용', exact: true })).toBeEnabled(); };
...
test('real HTTP and Firestore: generate, multi-file diff, apply, helper edit, preview, save, reopen and restore', async ({ page }) => {
  ...
  await page.getByLabel('새 파일 경로').fill('Obsolete.ts'); await page.getByRole('button', { name: '파일 추가', exact: true }).click();
  await page.getByLabel('React 원문').fill('export const old = true;');
  await generate(page);
  ...
  await page.getByRole('tab', { name: 'lib/label.ts', exact: true }).click(); await expect(page.getByLabel('React 원문')).toHaveValue(helper);
  const editedHelper = 'export const label = "실무자 편집 반영";'; await page.getByLabel('React 원문').fill(editedHelper);
  ...
  await page.getByRole('button', { name: 'React 저장·PR 생성' }).click(); await expect(page.getByText(/버전 1으로 저장했습니다/)).toBeVisible();
  await page.reload(); await page.getByRole('button', { name: /여러 파일로 만든 업무 화면.*버전 1/ }).click();
  await page.getByRole('tab', { name: 'lib/label.ts', exact: true }).click(); await expect(page.getByLabel('React 원문')).toHaveValue(editedHelper);
  await page.getByLabel('React 원문').fill('export const label = "두 번째 버전";');
  await page.getByRole('button', { name: 'React 저장·PR 생성' }).click(); await expect(page.getByText(/버전 2으로 저장했습니다/)).toBeVisible();
  await page.getByRole('button', { name: 'React 버전 이력' }).click();
  const row = page.locator('.history-row').filter({ hasText: '버전 1' }); await row.getByRole('button', { name: 'React 버전 복원' }).click();
  await expect(page.getByText(/새 버전 3으로 복원/)).toBeVisible(); await page.getByRole('tab', { name: 'lib/label.ts', exact: true }).click(); await expect(page.getByLabel('React 원문')).toHaveValue(editedHelper);
...
test('stale title and hidden file block atomic apply; compiler diagnostics preserve last good preview', async ({ page }) => {
  await page.goto('/?mode=react'); await expect(page.getByLabel('React 원문')).not.toHaveValue(''); await generate(page);
  const before = await page.getByLabel('React 원문').inputValue();
  await page.getByLabel('React 화면 제목').fill('제안 뒤 직접 바꾼 제목');
  await expect(page.getByRole('button', { name: 'React 제안 적용', exact: true })).toBeDisabled(); await expect(page.getByLabel('React 원문')).toHaveValue(before);
  await page.getByRole('button', { name: '제안 닫기', exact: true }).click(); await generate(page); await page.getByRole('button', { name: 'React 제안 적용', exact: true }).click();
  await page.getByRole('button', { name: 'React 미리보기 적용' }).click(); const frame = page.frameLocator('[data-testid="react-preview-committed"]');
  await expect(frame.getByRole('heading', { name: '파일 연결 확인' })).toBeVisible();
  await generate(page); await page.getByRole('tab', { name: 'lib/label.ts', exact: true }).click(); await page.getByLabel('React 원문').fill('export const label: string = 12;');
  await page.getByRole('tab', { name: 'App.tsx · 시작', exact: true }).click();
...
test('long file deletion can be reviewed fully before applying any proposal', async ({ page }) => {
  ...
  await page.getByLabel('React 원문').fill(old); await generate(page);

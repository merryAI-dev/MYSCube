...
export function ReactConversationPanel({ modelEnabled, busy, currentSource, apis, onProposal, onRestoreContext }: Props) {
  ...
  const refreshList = async () => { const list = await request(''); if (mounted.current) setSessions(list.items || []); };
  ...
  useEffect(() => {
    ...
    void refreshList().catch((reason) => { if (mounted.current) setError(reason.message); });
    if (id && /^[a-f0-9-]{36}$/i.test(id)) { setLoading(true); void hydrate(id, at, true).then((value) => { if (mounted.current && at === generation.current) setMode(value.lastMode || 'react'); }).catch((reason) => { if (mounted.current) setError(reason.message); }).finally(() => { if (mounted.current) setLoading(false); }); }
  ...
  const open = async (id: string) => {
    ...
    try { const value = await hydrate(id, at, true); if (mounted.current && generation.current === at) setMode(value.lastMode || 'react'); }
    catch (reason) { if (mounted.current && generation.current === at) setError(reason instanceof Error ? reason.message : '대화를 열지 못했습니다.'); }
    finally { working.current = false; if (mounted.current && generation.current === at) setLoading(false); }
  };
  const newConversation = () => { if (working.current || disabled) return; generation.current++; setSession(null); setStoredContext(null); setMessage(''); setError(''); setMode('react'); rememberUrl(null); };
  const send = async (answer?: string, clarificationId?: string) => {
    ...
    try {
      ...
      const response = await request(`/${active!.id}/turns`, 'POST', { expectedVersion: active!.version, requestId: crypto.randomUUID(), message: text, mode, ...(!usingStoredContext ? { currentSource, apis } : {}), ...(clarificationId ? { clarificationId } : {}) });
  ...
  return <section className="side-panel conversation-panel" aria-label="React 제작 대화">
    <div className="section-heading"><h2>이어서 만드는 업무 대화</h2><button className="quiet compact" disabled={disabled} onClick={newConversation}>새 제작 대화</button></div>
    <p className="subtle">자료 질문과 화면 제작을 같은 대화에서 이어갑니다. 질문에 답하거나 대화를 다시 열어도 편집 중인 코드는 자동으로 바뀌지 않습니다.</p>
    ...
    <label>이번 요청<select style={{ width: '100%', minWidth: 0 }} aria-label="제작 대화 요청 종류" disabled={disabled} value={mode} onChange={(event) => setMode(event.target.value as Mode)}><option value="react">React 화면 만들기·수정</option><option value="analysis">자료·오류 원인 질문</option></select></label>
    <textarea className="prompt" aria-label="React 화면 요청" disabled={disabled || !modelEnabled} value={message} onChange={(event) => setMessage(event.target.value)} placeholder={mode === 'react' ? '앞서 확인한 자료를 월별 필터가 있는 업무 화면으로 만들어 주세요.' : '자료를 확인하고 필요한 조건부터 질문해 주세요.'} />
    <button disabled={disabled || !modelEnabled || !message.trim()} onClick={() => void send()}>{loading ? '대화를 처리하고 있습니다…' : mode === 'react' ? 'React 생성·수정 요청' : '자료 질문 보내기'}</button>

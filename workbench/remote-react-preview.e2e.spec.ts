...
async function setup(page: Page) {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (path === '/api/v1/react-work-pages' || path.endsWith('/conversations') || path === '/api/v1/workbench-apis') return reply({ items: [] });
    return reply({ message: `Unexpected mocked endpoint ${path}` }, 404);
  });
  await page.goto('/?mode=react'); await expect(page.getByLabel('React 원문')).toHaveValue(source);
...
test('outdated frames cannot roll back pixels or evidence; failed input preserves its text and requires a fresh check', async ({ page }) => {
  const state = await setup(page); await page.getByRole('button', { name: 'React 미리보기 적용' }).click();
...
test('a candidate completed after leaving the editor is closed instead of being displayed', async ({ page }) => {
  const state = await setup(page); await page.getByRole('button', { name: 'React 미리보기 적용' }).click(); await expect(page.getByTestId('remote-react-frame')).toHaveAttribute('data-session', idA);
  let release: () => void = () => {}; state.holdCreate = new Promise<void>((resolve) => { release = resolve; }); state.returnedId = idC;
  await page.getByRole('button', { name: 'React 미리보기 적용' }).click(); await expect(page.getByText(/새 실행 화면을 준비하고 있습니다/)).toBeVisible();
  await page.getByRole('button', { name: '새 React 화면', exact: true }).click(); await expect(page.getByTestId('remote-react-frame')).toHaveCount(0);
...
test('a permission failure hides the previous pixels and evidence and retires the live session', async ({ page }) => {
  ...
  await expect(page.getByRole('region', { name: '격리된 React 실행 화면' }).getByRole('alert')).toContainText('이전 실행 이미지와 계산 근거를 숨겼습니다');

...
export async function runRendererWorker({ input = process.stdin, output = process.stdout, packageFile = new URL('./packages.json', import.meta.url), launch = (options) => chromium.launch(options) } = {}) {
  ...
  const screenshot = async (requestId) => {
    const png = await page.screenshot({ type: 'png', timeout: 5000, animations: 'disabled' });
    send({ type: 'frame', requestId, sequence: ++sequence, width: viewport.width, height: viewport.height, pngBase64: png.toString('base64') });
  };
  ...
  const command = async (message) => {
    ...
    try {
      if (message.type === 'init') await init(message);
        ...
        const event = checkEvent(message.event, viewport);
  ...
  reader.on('line', (line) => {
    ...
    void command(message).catch(async (error) => { try { send({ type: 'error', requestId: message.requestId, message: String(error.message || '실행 오류').slice(0, 500) }); } finally { await stop(); } }).finally(() => { inFlight--; });
  });

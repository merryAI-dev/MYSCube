...
export function createQaEvidenceService({ db, now = () => new Date().toISOString(), readCode = createGithubCodeReader({ now }) }) {
  return async (context, raw, signal = AbortSignal.timeout(20000)) => {
    ...
    const server = operationDocs.length <= 10 && operations.length === 1 ? operations[0] : null;
    ...
    return { question: input.question, answerMode: 'deterministic_evidence', queriedAt: now(), facts, candidates,
      unknowns: ['요청 번호는 호출자가 지정할 수 있습니다. 계정·업무·시각까지 일치해도 인과관계 확정은 아닙니다.', '코드 위치 일치만으로 근본 원인을 확정하지 않습니다.',
        ...(operations.length > 1 || operationDocs.length > 10 ? ['여러 서버 기록이 겹치거나 조회 한도에 도달하여 코드 버전을 선택하지 않았습니다.'] : []),
      ...
      logs, operations, github, correlation: server ? 'REQUEST_METADATA_CANDIDATE' : 'NOT_ESTABLISHED',
...
export function mountQaEvidenceRoutes(app, { db, now, asyncHandler, readCode }) {

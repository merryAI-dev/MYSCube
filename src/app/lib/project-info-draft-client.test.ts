...
describe('project information draft client', () => {
 ...
 it('loads older history and restores only a server revision with lease and CAS protection', async () => {
   ...
   expect(api.get).toHaveBeenCalledWith('/api/v1/project-info-drafts/project-a/history?beforeRevision=20', expect.any(Object));
   vi.mocked(api.post).mockReset().mockResolvedValueOnce({ data: { draft: { ...DRAFT, draftRevision: 31 } } } as never);
   const restored = await client.restoreHistory({ leaseId: 'lease-a', fence: 3 }, { revision: 12, expectedDraftRevision: 30, historyGeneration: 'generation-a' }, 'restore-a');
   expect(restored.draft.draftRevision).toBe(31);
   expect(api.post).toHaveBeenCalledWith('/api/v1/project-info-drafts/project-a/history/12/restore', expect.objectContaining({
   ...
   await expect(client.restoreHistory({ leaseId: 'lease-a', fence: 3 }, { revision: 12, expectedDraftRevision: 30, historyGeneration: 'generation-a' }, 'restore-b')).rejects.toThrow('draft_version_conflict');
 });

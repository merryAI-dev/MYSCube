...
- Vercel 활성화는 GitHub Production variable `SETTLEMENT_HERMES_URL`로 한다. 비워두면 기존 harness가 유지된다. 서비스 검증 후 값 설정 및 Production Deploy의 사유 있는 재시도로 활성화한다.
...
- 실제 Gemini API / Cloud Run / Slack E2E: 배포 후 별도 기록해야 한다. 모의 응답으로 완료 판정하지 않는다.
- 조감도는 `agent_capabilities`에서 제공한다. 연결 시트 직접 읽기와 정산 의무/종료 제외 정책은 아직 지원하지 않는다고 명시한다.

## 실제 E2E 통과 기준

1. 인증 없는 서비스 호출 403, 공개 IAM 없음, runtime 업무 DB 역할 없음.

...
- [x] 개인 임시저장 입력 원문 보존 및 최종 제출 정규화 분리 (개발 반영, 미배포)
...
- [2026-09-21 / 미배포] 등록 초안 생성·저장에서 제출용 변환을 분리했다. 입력한 문구와 첨부 참조를 초안으로 보존하고 최종 제출할 때 기존 업무 검증을 적용한다. 숫자 형식 오류는 기존 저장 차단·로컬 보존 정책을 유지한다. 운영 데이터 일괄 수정 없이 개발·검증 중이며 실제 배포는 별도 진행한다.

...
- [x] 임시저장 실제 이력 조회·버전별 비교 패널, 저장 확인본과 제출 문서 구분 (개발 반영, 미배포)
- [x] 필수 금액의 미입력/명시적 0 구분, 조건부 필수·해당 없음 응답 보존 (개발 반영, 미배포)

- [x] 정산 적용 사업의 이자 반납 여부 최종 제출 필수, 사업비 입력 방식 현재 설정 표시 (작업 트리 반영, 미배포)
...
- 2026-09-21: 이자 미선택은 임시저장 허용·최종 제출 차단으로 구분하고, 사업비 입력 방식을 읽기전용으로 표시. 전체 필수값 감사에서 확인한 금액 미입력→0 문제는 별도 미해결 항목으로 기록.

...
## [2026-09-21] 개발 기록 | 프로젝트 초안·제출·검토 버전 정합성 (미배포)

- pages: [등록](./pages/portal-register-project.md), [수정](./pages/portal-edit-project.md), [승인](./pages/admin-project-approvals.md)
- 원인: 개인 초안에 최종 제출 변환을 재사용하고, 승인 시 검토한 버전과 실제 반영 버전을 연결하지 못했으며, 화면별 필드·기본값·계산 기준이 달랐다.
- 조치: 초안 전용 저장, 제출 필드 소유권 계약, 읽은 문서의 서버 토큰 대조, 공통 표시·금액 확인, 이전 형식/첨부 사전 안내를 연결했다.
- 검증: 격리 저장 왕복·Firestore emulator·실제 React 브라우저와 운영 저장본의 읽기 전용 재현을 구분해 확인한다. 운영 데이터 쓰기·머지·배포 없음. 상세 결과는 스프린트 QA 보고서에 기록한다.

### 2026-09-21 — 기타 역할 추가 및 임시저장 보존 (미배포)

- 원인: 새 빈 역할 행이 화면 갱신 시 제출용 정규화에 의해 즉시 제거됨.
- 조치: 초안 전용 행 보존 옵션으로 빈 줄·입력 중 공백을 유지. 최종 제출 정책과 주정산·월결산 파이프라인은 변경하지 않음.

### 2026-09-21 — 이자 선택 검증 및 전체 필수값 감사 (미배포)

- 이자 반납 여부는 정산 적용 시 새 최종 제출에서 필수. 임시저장과 기존 승인 문서에 소급하지 않음. 사업비 입력 방식은 현재 설정 읽기전용 표시.
- 전체 감사에서 원가 미입력이0으로 바뀌고 입력완료 상태가 일괄설정되는 결함을 재현. 전체 필수값 개선은 미완료. docs/qa/2026-09-21-required-fields-audit.md 참조.
